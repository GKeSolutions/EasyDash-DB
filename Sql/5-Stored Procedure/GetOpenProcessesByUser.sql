IF OBJECT_ID('[ed].[GetOpenProcessesByUser]','P') IS NULL
	EXEC('CREATE PROCEDURE [ed].[GetOpenProcessesByUser] WITH ENCRYPTION AS BEGIN SELECT NULL; END;');
GO
    ALTER PROCEDURE [ed].[GetOpenProcessesByUser] @userId uniqueidentifier
    AS
    BEGIN
        SET NOCOUNT ON;
        SELECT
