...
IF OBJECT_ID('[ed].[GetMissingTimeUsersPerRole]','P') IS NULL
	EXEC('CREATE PROCEDURE [ed].[GetMissingTimeUsersPerRole] WITH ENCRYPTION AS BEGIN SELECT NULL; END;');
GO
ALTER PROCEDURE [ed].[GetMissingTimeUsersPerRole]
	@StartDate DateTime
	, @EndDate DATETIME
